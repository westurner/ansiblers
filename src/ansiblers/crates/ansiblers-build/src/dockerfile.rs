...
Self { name: name.into(), from: from.into(), instructions: Vec::new() }
}
...
self.instructions.push(Instruction::Arg(name.into(), default));
self
}
...
caches.iter().map(|(p, id)| (p.to_string(), id.to_string())).collect();
self.instructions.push(Instruction::RunCached { cmd: cmd.into(), caches: mounts });
self
}

/// Append a `COPY` instruction.
pub fn copy(&mut self, src: impl Into<String>, dst: impl Into<String>) -> &mut Self {
self.instructions.push(Instruction::Copy { src: src.into(), dst: dst.into(), from_stage: None });
self
}
...
self.instructions.push(Instruction::Entrypoint(args.iter().map(|s| s.to_string()).collect()));
self
}

/// Append a `CMD` instruction (exec form).
pub fn cmd(&mut self, args: &[&str]) -> &mut Self {
self.instructions.push(Instruction::Cmd(args.iter().map(|s| s.to_string()).collect()));
self
}
...
Run(String),
RunCached { cmd: String, caches: Vec<(String, String)> },
Copy { src: String, dst: String, from_stage: Option<String> },
Env(String, String),
...
Instruction::Copy { src, dst, from_stage: None } => format!("COPY {src} {dst}"),
Instruction::Copy { src, dst, from_stage: Some(stage) } => {
...
stage.run_cached("cargo build --release", &[("/usr/local/cargo/registry", "cargo-reg")]);
builder.add_stage(stage);
let text = builder.render();
